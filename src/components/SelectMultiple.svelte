<script>
  ...
  function removeExtraContentStyle() {
    ...
    extraContent.forEach((content) => {
      if (content.classList.contains("active")) {
        const iconUp =
          content.previousElementSibling.children[0].children[0].children[0]
            .children[1];
        const iconDown =
          content.previousElementSibling.children[0].children[0].children[0]
            .children[0];
  ...
  $: if (selectedValue.length > 0 && selectedValue !== "") {
    ...
    tagsHTML.innerHTML =
      firstSelected +
      '<span id="whatever" style="color: #0054A4" class="select__select-tag__counter"></span>';
    selectedCounter = selectedValue.length - 1;
    selectLabel = firstSelected;
    isTagSelected = !isTagSelected;
    spanElement = document.querySelector("#whatever");
    if (selectedCounter > 1) {
      selectedTagCounterText =
        '<span class="select__select-tag__counter>+' +
        selectedCounter +
        "</span>";
...
<div class="select__select-wrapper" bind:this={spanHTML}>
  <div
    id="selectTags"
    class="select__select-tags-container"
  ...
  >
    ...
    <span class="select__select-tag__counter"
      >{selectedCounter > 0 ? "+" + selectedCounter : ""}</span
    >
    <span class="select__select-tag-icon">{@html chevron}</span>
  </div>
  <div
    class="select__tags-options hide"
